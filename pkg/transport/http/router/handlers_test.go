...
func (s stubResponderProvider) Responder(_ http.ResponseWriter, _ *http.Request) response.Responder {
	return s.responder
}
...
func TestNew404Handler(t *testing.T) {
	...
	router.New404Handler(responderProvider).
...
func TestNew405Handler(t *testing.T) {
	...
	router.New405Handler(responderProvider).
		ServeHTTP(httptest.NewRecorder(), req)
	if timesCalled != 1 {
		t.Fatalf("expected to be called 1 time, called (%d) time(s)", timesCalled)
	}
